...
class FacebookPostService
{
    ...
    public function productRole($product,$access_code = null)
    {
        ...
        if($access_code){
            ...
            {
                $message[] = 'ยอดขายขั้นต่ำ '. (float) $product['ratingStar'] .' มีค่าน้อยกว่าค่าที่ตั้งไว้ ' . $saled;
            ...
            if ($product['ratingStar'] < 4.5 || $product['sales'] < 5) $x++;
        }
    ...
    public function postFeeds($data,$shopee_page = 1)
    {
        // try {
            ...
            $imageResponse = $facebook->userPostFeed($caption, $productImage);
            ...
            Log::channel('shopee_log')->error('แชร์สินค้าล้มเหลว: '. jsonEncode(['itemId' => $data['itemId'], 'error' => $errorDetails]));
            return ['status' => false];
        // } catch (Exception $e) {
